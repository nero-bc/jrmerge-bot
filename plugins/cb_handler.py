    ...
    if cb.data == "merge":
        await cb.message.edit(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("🛟 Yeah Upload", callback_data="to_telegram"),
                    ],
                    [
        ...
        await cb.message.edit(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("Default 📚", callback_data="rename_NO"),
                        InlineKeyboardButton("Rename ✍️", callback_data="rename_YES"),
                    ],
                    [InlineKeyboardButton("📴 Cancel", callback_data="cancel")],
                ]
            ),
        ...
        if "YES" in cb.data:
            await cb.message.edit(
                "<b>Current filename: (@jr_bots)_merged.mkv\n\nSend me new file name without extension: (e.g- .mkv, .mp4).</b>"
            )
        ...
        if "NO" in cb.data:
            new_file_name = (
                f"downloads/{str(cb.from_user.id)}/(@jr_bots)_merged.mkv"
            )
            if user.merge_mode == 1:
                await mergeNow(c, cb, new_file_name)
            elif user.merge_mode == 2:
            ...
            try:
                await cb.message.edit(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton("🗑️ Remove", callback_data=f"removeFile_{str(m.id)}"),
            ...
            try:
                await cb.message.edit(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton("🗑️ Remove", callback_data=f"removeFile_{str(m.id)}",),
                                InlineKeyboardButton("🗑️ Remove Subtitle", callback_data=f"removeSub_{str(sIndex)}"),
                            ],
                            [
                                InlineKeyboardButton("🔙 Back", callback_data="back")
                            ],
                ...
                await cb.message.edit(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                           [
                                InlineKeyboardButton("🗑️ Remove File", callback_data=f"removeFile_{str(m.id)}"),
                                InlineKeyboardButton("🗑️ Remove Subtitle", callback_data=f"removeSub_{str(sIndex)}")
                            ],
                            [
                                InlineKeyboardButton("🔙 Back", callback_data="back")
