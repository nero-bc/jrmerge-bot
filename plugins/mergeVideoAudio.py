...
async def mergeAudio(c: Client, cb: CallbackQuery, new_file_name: str):
    ...
    for i in msgs:
        ...
        try:
            ...
            if gDict[cb.message.chat.id] and cb.message.id in gDict[cb.message.chat.id]:
                return
            await cb.message.edit(f"**Video is downloaded successfully\n {media.file_name}**")
    ...
    if muxed_video is None:
        ...
        return
    try:
        await cb.message.edit("**Video is successfully muxed !**")
    except MessageNotModified:
        await cb.message.edit("**Video is sucessfully muxed !**")
    ...
    if file_size > 2147483648 and Config.IS_PREMIUM == False:
        await cb.message.edit(f"<b>Video is Larger than 2GB Can't Upload\nContact : @{Config.OWNER_USERNAME} to purchase premium membership for 4GB support.</b>")
        await delete_all(root=f"downloads/{str(cb.from_user.id)}")
        queueDB.update({cb.from_user.id: {"videos": [], "subtitles": [], "audios": []}})
        formatDB.update({cb.from_user.id: None})
        return
